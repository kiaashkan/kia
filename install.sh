...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/kiaashkan/kia/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
...
if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/kiaashkan/kia/releases/download/${last_version}/x-ui-linux-$(arch3xui).tar.gz"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/kiaashkan/kia/main/x-ui.sh
